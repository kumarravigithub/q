MetaSetting = {
  uploadPath: "/home/qart/files",
  reportPath: "/home/qart/files/reports"
}
...
Images = new FilesCollection({
  ...
  storagePath: MetaSetting.uploadPath,
...
if (Meteor.isServer) {
  ...
  Meteor.publish('reportstatus', function () {
    return tasks.find({});
  });
...
checkTasks = function () {
  ...
  var ispresent = tasks.find({ type: "ABCREPORT", status: "pending" }, { $limit: 1 }).fetch();
  if (ispresent.length > 0) {
    fileid = ispresent[0].fileid;
    var filedata = Images.find({}).fetch();
    ...
    taskList = ispresent;
    isTaskRunning = true;
    report1(filedata[0], ispresent[0].season);
  }
}

Meteor.setInterval(checkTasks, 1000);
