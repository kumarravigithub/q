      ...
      for (l = 0; l < result.length; l++) {
        ...
        for (i = 0; i < finalrows.length; i++) {
          ...
          switch (finalrows[i].gencat2) {
            case "MB":
              MB.all.push(finalrows[i])
              break;
            case "MT":
              MT.all.push(finalrows[i])
              break;
            case "WT":
              WT.all.push(finalrows[i])
              break;
            case "WB":
              WB.all.push(finalrows[i])
              break;
            default:
          }
          switch (true) {
        ...
        // ** SUM of MB **
        MB.all.sort((a, b) => Number(b.netretailvalue) - Number(a.netretailvalue));
        MT.all.sort((a, b) => Number(b.netretailvalue) - Number(a.netretailvalue));
        WT.all.sort((a, b) => Number(b.netretailvalue) - Number(a.netretailvalue));
        WB.all.sort((a, b) => Number(b.netretailvalue) - Number(a.netretailvalue));
        sumofMB=sumofABC(MB);
        sumofMT=sumofABC(MT);
        sumofWT=sumofABC(WT);
        sumofWB=sumofABC(WB);
        sumofpercent=0;
        for(i=0;i<MB.all.length;i++) {
          sumofpercent+=(finalrows[i].netretailvalue / sumofsellthroughdiscount);
          switch (true) {
            case sumofpercent <= 0.80:
              analysisA.push(finalrows[i]);
              break;
...
function sumofABC(gencat) {
  sumofGencat = 0;
  for (i = 0; i < gencat.all.length; i++) {
    sumofGencat += gencat.all[i].netretailvalue;
  }
  return sumofGencat;
}
